...
def test_get_classification_report():
    ...
    expected_output = {}
    expected_output["precision_macro"] = "{:.4f}".format(report["macro avg"]["precision"])
    expected_output["recall_macro"] = "{:.4f}".format(report["macro avg"]["recall"])
    expected_output["f1_macro"] = "{:.4f}".format(report["macro avg"]["f1-score"])
    expected_output["precision_weighted"] = "{:.4f}".format(report["weighted avg"]["precision"])
    expected_output["recall_weighted"] = "{:.4f}".format(report["weighted avg"]["recall"])
    expected_output["f1_weighted"] = "{:.4f}".format(report["weighted avg"]["f1-score"])
    expected_output["accuracy_score"] = "{:.4f}".format(accuracy_score(y_test, y_pred))
    expected_output["matthews_correlation_coefficient"] = "{:.4f}".format(matthews_corrcoef(y_test, y_pred))
